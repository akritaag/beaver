...
class REFORCE:
    ...
    def self_refine(self, args, logger, question, format_csv, table_struct, table_info, response_pre_txt, pre_info, csv_save_path, sql_save_path, task=None):
        ...
        if self.option is not None:
            ...
            try:
                ...
                preprocessing_option = 4
        else:
            preprocessing_option = 4

        self_refine_prompt = self.prompt_class.get_self_refine_prompt(table_info, task, pre_info, question, self.api, format_csv, table_struct, args.omnisql_format_pth, db_id=self.db_id, option=preprocessing_option)
    ...
    def gen(self, args, logger, question, format_csv, table_struct, table_info, response_pre_txt, pre_info, csv_save_path, sql_save_path, task=None):
        ...
        gen_prompt = self.prompt_class.get_self_refine_prompt(table_info, task, pre_info, question, self.api, format_csv, table_struct, args.omnisql_format_pth, db_id=self.db_id, option=preprocessing_option)
        logger.info("[Gen]\n" + gen_prompt + "\n[Gen]")
        max_try = self.max_try
        while max_try > 0:
