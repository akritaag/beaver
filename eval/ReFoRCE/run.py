...
def process_sql_data(sql_data):
    ...
    # Get Beaver gold res
    if args.task == "beaver" and sql_data in full_gold_sql:
        os.makedirs(args.gold_result_path, exist_ok=True)
        gold_pth = os.path.join(args.gold_result_path, sql_data+".csv")
        if not os.path.exists(gold_pth):
            from utils import get_api_name
            sql_env = SqlEnv()
            api = get_api_name(sql_data)
            res = sql_env.execute_sql_api(full_gold_sql[sql_data], sql_data, gold_pth, api=api, timeout=1200)
            if res != "0":
                print(sql_data, res)
    ...
    if args.do_vote:
        ...
        # wait with timeout (4 minutes per thread)
        TIMEOUT_SECONDS = 240  # 4 minutes
        for i, thread in enumerate(threads):
            thread.join(timeout=TIMEOUT_SECONDS)
            if thread.is_alive():
                print(f"⚠ Warning: Thread {i} for {sql_data} exceeded {TIMEOUT_SECONDS}s timeout, skipping...")
