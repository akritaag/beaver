...
class SqlEnv:
    ...
    def start_db_mysql(self, ex_id):
        ...
        if ex_id not in self.conns.keys():
            ...
            cred_path = os.environ.get("MYSQL_CREDENTIAL_PATH")
            if cred_path and os.path.exists(cred_path):
                mysql_credential = json.load(open(cred_path))
            else:
            ...
            self.conns[ex_id] = mysql.connector.connect(**mysql_credential)
    ...
    def exec_sql_mysql(self, sql_query, save_path=None, max_len=30000, ex_id=None):
        """Execute SQL query on MySQL"""
        # Check if connection exists and is alive, reconnect if needed
        if ex_id not in self.conns or not self.conns[ex_id].is_connected():
            print(f"MySQL connection for {ex_id} is not available, reconnecting...")
            if ex_id in self.conns:
                try:
        ...
        cursor = self.conns[ex_id].cursor(buffered=True)
        try:
            cursor.execute(sql_query)
            column_info = cursor.description
            # Fetch all rows at once to avoid "Unread result found" error
