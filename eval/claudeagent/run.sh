...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --q_fn)        Q_FN="$2";        shift 2 ;;
        --help|-h)
            echo "Usage: ./run.sh --dataset <ds> --setting <0|1|2> [--num_workers N] [--q_fn FILE]"
            echo "  --model        run label for the output dir (default: claude)."
            echo "                 Backend = claude -p; set CLAUDE_MODEL to pick the model."
