...
def hard_prompt_maker(row, schema_links, sub_questions, spider_schema, spider_foreign, args, table_filter=None):
    ...
    for db_name in db_names:
        fields = find_fields_MYSQL_like(db_name, spider_schema, table_filter)
        if row.get('join_keys'):
            foreign_keys = format_join_keys(row['join_keys'])
        else:
...
def classification_prompt_maker(test_sample_text, db_names, schema_links, spider_schema, spider_foreign, join_keys=None, table_filter=None):
    # done: support multi-db
    if isinstance(db_names, str):
        db_names = [db_names]
    ...
    for db_name in db_names:
        fields = find_fields_MYSQL_like(db_name, spider_schema, table_filter)
        if join_keys:
            foreign_keys = format_join_keys(join_keys)
        else:
            foreign_keys = find_foreign_keys_MYSQL_like(db_name, spider_foreign)
...
def format_join_keys(join_keys):
    if not join_keys:
        return "[]"
    ...
    for jk in join_keys:
        if len(jk) == 2:
            formatted.append(f"{jk[0]} = {jk[1]}")
    
    return "[" + ",".join(formatted) + "]"

def schema_linking_prompt_maker(test_sample_text, db_names, spider_schema, spider_foreign, join_keys=None, table_filter=None):
  # done: support multi-db
  if isinstance(db_names, str):
      db_names = [db_names]
  ...
  return prompt
def find_fields_MYSQL_like(db_name, spider_schema, table_filter=None):
  df = spider_schema[spider_schema['Database name'] == db_name]
  df = df.groupby(' Table Name')
  output = ""
  if table_filter:
      table_filter = set([t.lower() for t in table_filter])
  for name, group in df:
    if table_filter and name.lower() not in table_filter:
        continue
...
def debuger(test_sample_text, db_names, sql, spider_schema, spider_primary, spider_foreign, join_keys=None, table_filter=None):
    # done: support multi-db
    if isinstance(db_names, str):
        db_names = [db_names]

    instruction = """#### For the given question, use the provided tables, columns, and foreign keys to fix the given SQL QUERY for any issues. If there are any problems, fix them. If there are no issues, return the SQL QUERY as is. You should return the FIXED SQL QUERY only, without any explanation.
...
"""
    ...
    prompt = instruction + all_fields + all_foreign_keys + '#### Question: ' + test_sample_text + '\n#### SQL QUERY\n' + sql + '\n#### FIXED SQL QUERY\nSELECT'
    
    return prompt


def GPT4o_generation(prompt, n=1, temperature=0, model="gpt-4.1"):
  init_client(model)
  global CLIENT
  try:
    # print(f"model: {model}")
    if "gemini" in model:
        try:
            ...
            print(f"Gemini call failed with reasoning_effort='none', retrying without. Error: {e}")
            response = CLIENT.chat.completions.create(
                model=model,
                messages=[{"role": "user", "content": prompt}],
                stream = False,
...
def GPT4o_debug(prompt, n=1, temperature=0, model="gpt-4.1"):
  init_client(model)
  global CLIENT
  try:  
    if "gemini" in model:
