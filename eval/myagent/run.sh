...
#   uv run python evaluate_subtasks.py --dataset <ds> --model gpt-5-mini --input_dir unified-output/myagent/<run>
# ============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            echo "Usage: ./run.sh --model <label> --dataset <dataset> --setting <0|1|2> [--num_workers N]"
            echo ""
            echo "  --model        run label, used only for the output dir name (default: codex)."
            echo "                 Backend = Codex CLI; set CODEX_MODEL to pick the Codex model."
